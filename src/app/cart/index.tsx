...
export default function CartScreen() {
  ...
  // Reused state hooks strictly mimicking checkout setup context variables
  ...
  const [platformFeeFromConfig, setPlatformFeeFromConfig] = useState<number>(0);
  const [deliveryConfig, setDeliveryConfig] = useState<DeliveryConfig | null>(null);
  const [customerAddress, setCustomerAddress] = useState<{ latitude: number | null; longitude: number | null } | null>(null);
  ...
  // Exact distance lookup mapping node
    ...
    async function loadCartBillingContext() {
      try {
        ...
        if (feeSettings?.setting_value) {
          ...
          setPlatformFeeFromConfig(Number(parsedFee.platform_fee || 0));
        }
        ...
        console.error('Error loading checkout setup context inside cart:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Summary Computations strictly mirroring checkout.tsx conditional parameter structures
  const billingBreakdown = useMemo(() => {
    ...
    return calculateBilling({
      cartItems,
      distanceKm: distance, 
      platformFee: cartItems.length > 0 ? platformFeeFromConfig : 0,
      commissionPercent,
      deliveryConfig,
    });
    ...
    return (
      <SafeAreaView style={styles.safeContainer}>
        <View style={styles.header}>
          <View style={[styles.backButton, { backgroundColor: '#E2E8F0', borderColor: '#E2E8F0' }]} />
          <View style={{ gap: 6 }}>
            <View style={{ width: 120, height: 18, backgroundColor: '#E2E8F0', borderRadius: 4 }} />
            <View style={{ width: 80, height: 12, backgroundColor: '#F1F5F9', borderRadius: 4 }} />
          </View>
        </View>
        <View style={{ padding: 16, gap: 12 }}>
          {[1, 2, 3].map((i) => (
            <View key={i} style={[styles.cartCard, { opacity: 0.6, borderColor: '#F1F5F9' }]}>
              <View style={{ flexDirection: 'row', gap: 12 }}>
                <View style={{ width: 64, height: 64, backgroundColor: '#E2E8F0', borderRadius: 14 }} />
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptySubtitle}>
          Looks like it's empty! Fill it up with goods to get instant delivery at your doorstep right now.
        </Text>
        <Pressable
          onPress={() => router.back()}
          style={({ pressed }) => [styles.continueButton, pressed && styles.microInteraction]}
        >
          <Text style={styles.continueButtonText}>Fill It Up With Goods</Text>
        </Pressable>
      </View>
    );
  }
    ...
    <SafeAreaView style={styles.safeContainer}>
      <View style={styles.flexContainer}>
        ...
        <View style={styles.header}>
          ...
          <View>
            ...
            <Text style={styles.headerSubtitle}>{cartItems.length} {cartItems.length === 1 ? 'item' : 'items'} inside Rivo instant delivery</Text>
          </View>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.cartCard}>
              {/* Swipe/Dismiss Indicator Handle Layout Decoration */}
              <View style={styles.swipeLeftBarIndicator} />
              ...
              <View style={styles.cardMainContentArea}>
                ...
                {/* Item Details Metadata Context Info */}
                <View style={styles.itemMeta}>
                  <Text style={styles.itemName} numberOfLines={2}>
                    {item.name}
                  </Text>
                  <Text style={styles.itemPriceCalculation}>
                    ₹{item.price} <Text style={styles.mutedMultiplier}>×</Text> {item.quantity}
                  </Text>
                  <Text style={styles.itemTotalPrice}>₹{item.price * item.quantity}</Text>
                </View>

                {/* Action Stepper Panel Controls & Remove */}
                <View style={styles.rightActionPanelControlsColumn}>
                  <Pressable
                    onPress={() => {
                      removeFromCart(item.id);
                      refresh();
                    }}
                    style={({ pressed }) => [styles.removeButton, pressed && styles.opacityInteraction]}
                  >
                    <Text style={styles.removeButtonText}>🗑️</Text>
                  </Pressable>

                  {/* Quantity Stepper Controller Selector */}
                  <View style={styles.quantityContainer}>
                    <Pressable
                      ...
                      style={({ pressed }) => [styles.quantityControlBtn, pressed && styles.opacityInteraction]}
                    >
              ...
              <View style={styles.billContainer}>
                <View style={styles.billHeaderRow}>
                  ...
                  <View style={styles.instantBadge}>
                    <Text style={styles.instantBadgeText}>⚡ SECURE CHECKOUT</Text>
                  </View>
                </View>
                
                {/* Items Card Fee breakdown segment line row */}
                <View style={styles.billRow}>
                  <View style={styles.labelRowWithIcon}>
                    <Text style={styles.rowLeadIcon}>🛒</Text>
                    <Text style={styles.billLabel}>Items Total (Subtotal)</Text>
                  </View>
                  <Text style={styles.billValue}>₹{itemsTotal}</Text>
                </View>

                {/* Delivery Fee card breakdown segment line row */}
                <View style={styles.billRow}>
                  <View style={styles.labelRowWithIcon}>
                    <Text style={styles.rowLeadIcon}>🛵</Text>
                ...
                {/* Platform Fee card breakdown segment line row */}
                <View style={styles.billRow}>
                  <View style={styles.labelRowWithIcon}>
                    <Text style={styles.rowLeadIcon}>🏢</Text>
                ...
                <View style={styles.grandTotalRow}>
                  <View>
                    <Text style={styles.grandTotalLabel}>To Pay Amount</Text>
                    <Text style={styles.gstNotice}>Includes all legal regional retail GST</Text>
                  </View>
      ...
      <View style={styles.stickyFooter}>
        <Pressable
          ...
          style={({ pressed }) => [styles.checkoutButton, pressed && styles.microInteraction]}
        >
          <View style={styles.checkoutTextContainer}>
            ...
            <Text style={styles.checkoutActionText}>Proceed to Checkout ➔</Text>
          </View>
        </Pressable>
      </View>
    </SafeAreaView>
